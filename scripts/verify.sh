...
# roauth2 uses client_secret_basic: creds in HTTP Basic header, not the body.
JWT=$(curl -fsS -u "${OAUTH_CLIENT_ID}:${OAUTH_CLIENT_SECRET}" -X POST "$TOKEN_URL" \
  -d grant_type=client_credentials \
  -d "audience=${AUD}" \
  -d "scope=garden:write" | python3 -c 'import sys,json;print(json.load(sys.stdin)["access_token"])')
